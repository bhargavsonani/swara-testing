...
const CreateBilling = ({ token }) => {
  ...
  // Fetch total PDF count from backend - FIXED VERSION
  ...
  // Add item to current session only
  const addItem = async (generatePDF = false) => {
    // Validate all fields
    ...
    if (!itemDate) {
      toast.error('Please select a date for the item');
      return;
    }

    try {
      setPdfLoading(generatePDF);
      
      if (generatePDF) {
        // For PDF generation, send current session items to backend
        const response = await fetch(`${backendUrl}/items/addWithCurrentSessionPdf`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            ...(token && { 'Authorization': `Bearer ${token}` })
          ...
          body: JSON.stringify({
            ...
            quantity: quantity,
            rate: rate,
            date: itemDate, // Include the manual date
        ...
        if (data.status) {
          ...
          const itemToAdd = {
            ...
            quantity: quantity,
            rate: rate,
            date: itemDate, // Include the date
          ...
          if (data.pdfGenerated) {
            toast.success('Item added and PDF generated successfully!');
            // Refresh total PDF count and invoice number
            await fetchTotalPDFCount();
            await fetchSavedPDFs(creationId);
          } else if (data.pdfError) {
            toast.warning(`Item added but PDF generation failed: ${data.pdfError}`);
          } else {
        ...
        // Just add to current session without backend call
        const itemToAdd = {
          id: Date.now(), // Temporary ID for new session items
          description: description,
          quantity: quantity,
  ...
  // Generate PDF for current session items only
  const generatePDFForCurrentSession = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `invoice-${creationData.name.replace(/\s+/g, '-')}-${new Date().toISOString().split('T')[0]}.pdf`;
  ...
  const calculateCurrentSessionTotals = () => {
    ...
    const discount = subtotal * (discountPercentage / 100); // Use manual discount percentage
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                ...
                disabled={pdfLoading}
                className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white px-4 py-2 rounded-md transition duration-200 text-sm"
              >
                {pdfLoading ? 'Processing...' : 'Add & Generate PDF'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6">
          <h2 className="text-lg font-semibold mb-3 text-green-700">Current Bill Items</h2>
                ...
                {currentSessionItems.length > 0 ? (
                  currentSessionItems.map((item, index) => (
                    <tr key={item.id || index} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 border-b text-center">{formatDate(item.date)}</td>
